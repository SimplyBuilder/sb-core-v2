...
- **Restructured monorepo:** Modules moved from root directories (`dom-module/`, `event-module/`, `core-module/`) to `packages/`
...
- **Renamed singletons:** `SimplyBuilder` → `SBCore`, `SimplyBuilderDom` → `SBCoreDom`, `EventModule` → `SBCoreEvent`
- **`domModuleExtends()`:** Now expects `name: "SBCoreEvent"` instead of `"EventModule"`

### 🚀 Enhancements

- **rollup + terser:** All packages compile with rollup and terser minification (ecma: 2020)
